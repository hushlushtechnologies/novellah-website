...
export function Navbar() {
  ...
  // On every route change: close any open menus, AND force the window back
  // to the top of the new page. Next.js's <Link> is supposed to reset
  // scroll on navigation, but if anything upstream (scroll={false} on a
  // link, router.push with { scroll: false }, or the browser's own
  // scrollRestoration) leaves window.scrollY > 0, this sticky header stays
  // pinned at that same offset — which visually hides the top of the new
  // page's hero section underneath it. We also reset `scrolled` here
  // rather than waiting for the scroll listener, otherwise the navbar can
  // render in its "compact/scrolled" state on a page the visitor hasn't
  // actually scrolled yet.
  useEffect(() => {
