          ...
          <div className="overflow-hidden rounded-2xl bg-border ">
            <div className="flex items-center gap-8">
              ...
              <div className="flex flex-1 flex-col">
                <h3 className="whitespace-pre-line font-bold font-heading text-[24px] leading-tight text-foreground">
          ...
          <div className="rounded-2xl bg-background-light border border-border">
            {treatmentsMegaMenuQuickLinks.map((link) => (
              <Link key={link.href} href={link.href} className="flex items-center gap-4 p-4">
                <span className="flex border h-10 w-10 shrink-0 items-center justify-center rounded-full text-secondary">
                  <Clock size={16} />
                </span>
                ...
                {category.items.slice(0, 5).map((item) => (
                  <li key={item.slug}>
                    <Link
                      ...
                      className="font-body text-sm   text-foreground hover:text-foreground"
                    >
                      {item.label[locale]}
                    </Link>
                  </li>
